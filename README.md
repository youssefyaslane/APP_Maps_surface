...
- `GET /api/unmatched_roofs.csv?min_area=` — export CSV (lat, lon, surface, potentiel) des grands toits (Microsoft ou IA/tracé manuel, `min_area` en m², défaut 2000) sans aucune entreprise connue à proximité. Le flux normal part des entreprises pour chercher leur toit ; cette liste couvre l'angle mort inverse : de grands bâtiments détectés mais absents du fichier scraper. Sert de base pour rechercher manuellement (ou via un scraper Google Maps) quelle entreprise occupe ces coordonnées. Le point exporté n'est **pas** le centroïde géométrique brut (qui tombe hors du bâtiment sur ~22% des toits en L/U/complexes) mais un point recalculé pour toujours tomber à l'intérieur du polygone, garantissant que le bâtiment visé apparaît bien sous le point sur Google Maps
